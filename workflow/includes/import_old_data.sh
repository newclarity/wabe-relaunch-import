...
mysql -h ${TARGET_HOSTNAME} -u {$DB_USERNAME} -p${DB_PASSWORD} -e "INSERT INTO wp_posts (SELECT * FROM old_posts)" ${DB_NAME}

announce "Importing old post meta ..."
mysql -h ${TARGET_HOSTNAME} -u {$DB_USERNAME} -p${DB_PASSWORD} -e "INSERT INTO wp_postmeta (SELECT * FROM old_postmeta)" ${DB_NAME}

announce "Importing old terms..."
mysql -h ${TARGET_HOSTNAME} -u {$DB_USERNAME} -p${DB_PASSWORD} -e "INSERT INTO wp_terms (SELECT * FROM old_terms)" ${DB_NAME}

announce "Importing old term_taxonomy..."
mysql -h ${TARGET_HOSTNAME} -u {$DB_USERNAME} -p${DB_PASSWORD} -e "INSERT INTO wp_term_taxonomy (SELECT * FROM old_term_taxonomy)" ${DB_NAME}

announce "Importing old term relationships..."
mysql -h ${TARGET_HOSTNAME} -u {$DB_USERNAME} -p${DB_PASSWORD} -e "INSERT INTO wp_term_relationships (SELECT * FROM old_term_relationships)" ${DB_NAME}
